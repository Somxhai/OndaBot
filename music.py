...
class music_cog(commands.Cog):
    ...
    async def p(self, ctx, *args):
        ...
        if voice_channel is None:
            ...
            await ctx.send("เข้าห้องดิ ไม่เข้ามันเปิดไม่ได้ อย่าเอ๋อ")
        else:
    ...
    async def skip(self, ctx):
        if self.vc != "" and self.vc:
            ...
            await self.play_music()
    @commands.command(name="stop", help="stop music for playing ไงไอโง่")
    async def stop(self, ctx):
        if self.is_playing():
            await self.vc.stop()
        else:
            await ctx.send("ออนด้ายังไม่ได้เปิดเพลงเลย uwu")